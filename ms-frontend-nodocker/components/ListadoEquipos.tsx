...
export default async function ListadoEquipos() {
  const response = await fetch("http://localhost:8080/api/equipos", {next: { revalidate: 3600, tags: ['equipos']  }});
  const rawResponse = await response.json();
  const equipos = rawResponse.equipos;    

  const response2 = await fetch("http://localhost:8080/api/devoluciones/estados", {next: { revalidate: 3600, tags: ['estados']  }});
